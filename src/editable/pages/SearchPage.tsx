    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#fff7ee)] text-[var(--editable-page-text,#2f1d16)]">
        <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-xl border border-[#12156f22] bg-white p-6 shadow-[0_30px_90px_rgba(18,21,111,0.08)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#ffb52e]">{pagesContent.search.hero.badge}</p>
              <h1 className="mt-5 text-5xl font-black leading-tight tracking-tight sm:text-6xl">{pagesContent.search.hero.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 text-[#545b70]">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-end rounded-xl border border-[#12156f22] bg-[#f7f8fc] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-xl bg-[#ffb52e] px-6 text-sm font-black uppercase tracking-[0.18em] text-[#111111] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/listing" className="inline-flex items-center gap-2 rounded-xl border border-[#12156f22] bg-white px-5 py-3 text-sm font-black">Browse listings <ArrowRight className="h-4 w-4" /></Link>
          </div>
